...
Player::Player(IrrlichtDevice* irrDevice, ISceneManager* manager, IVideoDriver* videoDriver, Camera* myCamera) : device(irrDevice), smgr(manager), driver(videoDriver), camera(myCamera)
{
	...
	node->setRotation(core::vector3df(0, 0, 0.0f));
...
void Player::moveForward(f32 deltaTime)
{
	...
	node->setPosition(node->getAbsolutePosition() + (forwardDirection * deltaTime * 0.5));
	
	node->updateAbsolutePosition();
	forwardDirection.normalize();
	camera->getNode()->setPosition((-1.0f * forwardDirection * cameraDistance) + node->getAbsolutePosition());
	camera->getNode()->updateAbsolutePosition();
	camera->getNode()->setPosition(vector3df(camera->getNode()->getAbsolutePosition().X, camera->getNode()->getAbsolutePosition().Y + 80, camera->getNode()->getAbsolutePosition().Z));
	camera->getNode()->updateAbsolutePosition();
	camera->getNode()->setTarget(node->getAbsolutePosition());
}
...
void Player::moveBackward(f32 deltaTime)
{
	...
	node->setPosition(vector3df(node->getAbsolutePosition().X + (-1 * forwardDirection.X * deltaTime * 0.5), node->getAbsolutePosition().Y, node->getAbsolutePosition().Z + (-1 * forwardDirection.Z * deltaTime * 0.5)));
...
PowerBall* Player::Attack()
{
	...
	PowerBall* powerBall = new PowerBall(smgr, driver, node->getAbsolutePosition(), direction, 3.0f);
